...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn("bg-black/60 fixed inset-0 z-50   data-[state=open]:animate-overlayShow", className)}
    {...props}
  />
));
  ...
  ({ className, children, size = "medium", ...props }, ref) => (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ref={ref}
        className={cn(
          `fixed left-[50%] top-[50%] z-50 grid w-full translate-x-[-50%] translate-y-[-50%] gap-4 rounded-md bg-panel border border-gray-a3 shadow-md duration-200 
